...
def train_ffae_arrhythmia(X,y):
    '''trains a feedforward autoencoder of 1 encoder & 1 decoder'''

    X_train, X_val = get_ae_dataset(X,y)
    
    model = Sequential()
    model.add(Dense(64, input_dim=X_train.shape[1], kernel_regularizer=l2(0.1),
                    activation='relu'))   
    ...
    filepath = 'models/best_ffae_arrhythmia.h5'
...
def train_ffae_mammography(X,y):
    '''trains a feedforward autoencoder of 1 encoder & 1 decoder'''

    X_train, X_val = get_ae_dataset(X,y)
    
    model = Sequential()
    model.add(Dense(2, input_dim=X_train.shape[1], kernel_regularizer=l2(0.01),
                    activation='tanh'))   

    #model.add(Dense(2, kernel_regularizer=l2(0.01), activation='tanh')) 
    ...
    model.compile(loss='mean_squared_error', metrics=['mse'], optimizer = adam)
    filepath = 'models/best_ffae_mammography.h5'
...
def train_ffae_webpage(X,y):
    '''trains a feedforward autoencoder of 1 encoder & 1 decoder'''

    X_train, X_val = get_ae_dataset(X,y)
    
    model = Sequential()
    model.add(Dense(128, input_dim=X_train.shape[1], kernel_regularizer=l2(0.1),
                    activation='tanh'))   
    ...
    model.compile(loss='mean_squared_error', metrics=['mse'], optimizer = adam)
    filepath = 'models/best_ffae_webpage.h5'
    ...
    history = model.fit(X_train, X_train,\
              batch_size=256,epochs=300,shuffle=True,verbose=2,\
              validation_data=(X_val, X_val), 
              callbacks=[early_stopping_monitor,checkpoint])
...
def generate_synthetic_ff(name, X, y, X_train, y_train):
    '''generate data for the minority class using Feedforward autoencoder'''

    #train Feedforward AE  
    if name == 'arrhythmia':
        #train_ffae_arrhythmia(X,y)
        filepath = 'models/best_ffae_arrhythmia.h5'    
    if name == 'mammography':      
        train_ffae_mammography(X,y)
        filepath = 'models/best_ffae_mammography.h5'     
    if name == 'webpage':      
        train_ffae_webpage(X,y)
        filepath = 'models/best_ffae_webpage.h5' 
    ...
    synthetic_data = []
    for i in range (1):
        preds = model.predict(X_train_minority)   
        print(preds.shape)
...
def concat_train_data(X_train, y_train, X_test):
...
def train_classifier_ff_ae(name, X_train, y_train, X_test, y_test):
...
def evaluate_ff_ae(name, X_test, y_test):
    ...
    for clf_name, clf in zip(names, classifiers):
      ...
      model = pickle.load(open('pickles/model_ff_ae_'+str(name)+'_'+str(clf)+'.p', 'rb'))
...
for name in names:
    ...
    synthetic_data = generate_synthetic_ff(name,X, y, X_train, y_train)
    #concatenate all training data
    X_train_ff_ae, X_test, y_train_ff_ae = concat_train_data(X_train, y_train, X_test)
    #train the classifiers
    train_classifier_ff_ae(name, X_train_ff_ae, y_train_ff_ae, X_test, y_test)
    #evaluate the models 
    #evaluate_ff_ae(name, X_test, y_test)
