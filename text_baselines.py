...
def create_tweets_dataset(): 
    ...
    e = generate_sentim_features(texts, embeds)      
    X = np.nan_to_num(e)
...
def train_eval_classifier_unbal(X_train, y_train):
...
def train_eval_classifier(X_train, y_train):
    '''trains LogR/RF/SVM on training set & saves the models to pickles.
    if cl_bal=True, class_weight='balanced' for  LogR/SVM & balanced RF is trained
    instead of RF'''
...
embed, labels = create_tweets_dataset()
...
train_eval_classifier_unbal(X_train, y_train)
train_eval_classifier(X_train, y_train)
